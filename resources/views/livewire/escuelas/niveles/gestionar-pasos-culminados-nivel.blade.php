<div>
    <h6 class="mb-3">Pasos de Crecimiento al Culminar</h6>
                ...
                <tbody>
                    @foreach($draftItems as $item)
                        <tr>
                            ...
                            <td>
                                <button type="button" wire:click="confirmarEliminacionNivelPasoCulminado('{{ $item['temp_id'] }}')" class="btn btn-sm btn-icon btn-text-secondary text-danger rounded-pill">
                                    <i class="ti ti-trash"></i>
                                </button>
                                <input type="hidden" name="pasos_culminar[{{ $loop->index }}][paso_id]" value="{{ $item['paso_id'] }}">
                                <input type="hidden" name="pasos_culminar[{{ $loop->index }}][estado_id]" value="{{ $item['estado_id'] }}">
                            </td>
                        </tr>
                    @endforeach
                </tbody>
    ...
    @if(!$draftMode && $pasosCulminados->count() > 0)
        <div class="table-responsive">
            <table class="table table-sm table-hover">
                <thead>
                    <tr>
                ...
                <tbody>
                    @foreach($pasosCulminados as $paso)
                        <tr>
                            <td>{{ $paso->nombre }}</td>
                            <td>
                            ...
                            <td>
                                <button type="button" wire:click="confirmarEliminacionNivelPasoCulminado({{ $paso->id }})" class="btn btn-sm btn-icon btn-text-secondary text-danger rounded-pill">
                                    <i class="ti ti-trash"></i>
                                </button>
                            </td>
                        </tr>
