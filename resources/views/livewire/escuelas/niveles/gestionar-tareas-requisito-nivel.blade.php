<div>
    <h6 class="mb-3">Tareas de Consolidación Requeridas</h6>
                ...
                <tbody>
                    @foreach($draftItems as $item)
                        <tr>
                            ...
                            <td>
                                <button type="button" wire:click="confirmarEliminacionNivelTareaRequisito('{{ $item['temp_id'] }}')" class="btn btn-sm btn-icon btn-text-secondary text-danger rounded-pill">
                                    <i class="ti ti-trash"></i>
                                </button>
                                <input type="hidden" name="tareas_requisito[{{ $loop->index }}][tarea_id]" value="{{ $item['tarea_id'] }}">
                                <input type="hidden" name="tareas_requisito[{{ $loop->index }}][estado_id]" value="{{ $item['estado_id'] }}">
                            </td>
                        </tr>
                    @endforeach
                </tbody>
    ...
    @if(!$draftMode && $tareasRequisito->count() > 0)
        <div class="table-responsive">
            <table class="table table-sm table-hover">
                <thead>
                    <tr>
                ...
                <tbody>
                    @foreach($tareasRequisito as $tarea)
                        <tr>
                            ...
                            <td>
                                <button type="button" wire:click="confirmarEliminacionNivelTareaRequisito({{ $tarea->id }})" class="btn btn-sm btn-icon  text-danger rounded-pill">
                                    <i class="ti ti-trash"></i>
                                </button>
                            </td>
                        </tr>
