<div class="mb-5" x-data="{ formVisible: {{ $tareasRequisito->count() == 0 ? 'true' : 'false' }} }">
    ...
    <p class="text-dark small mb-3">Configura las tareas de consolidación que el usuario debe tener para poder
        inscribirse a este grado</p>
    ...
    <div x-show="formVisible" x-transition class="row g-3 mb-4 align-items-end">
        <div class="col-md-5 col-sm-12" wire:ignore>
            ...
            <select id="select-nivel-tarea-requisito" class="form-select select2-livewire border-1">
        ...
        <div class="col-md-5 col-sm-12" wire:ignore>
            ...
            <select id="select-nivel-estado-tarea-requisito" class="form-select select2-livewire border-1">
                <option value="">Selecciona una opción</option>
    ...
    @if ($tareasRequisito->count() > 0 || count($draftItems) > 0)
                    ...
                    <tbody>
                        @if ($draftMode)
                            @foreach ($draftItems as $index => $item)
                                <input type="hidden" name="tareas_prerrequisito[]"
                        ...
                        @else
                            @foreach ($tareasRequisito as $index => $tarea)
                                <tr>
                                    ...
                                    <td class="text-center align-middle">
                                        <button type="button"
                                            @click="confirmarEliminacionNivelTareaRequisito({{ $tarea->id }})"
                                            class="btn btn-link text-danger p-0">
                                            <i class="ti ti-trash fs-5"></i>
                                        </button>
                                    </td>
            ...
            <div class="mt-3">
                <a href="#" @click.prevent="formVisible = !formVisible" style="text-decoration: underline;">
                    <i class="ti ti-circle-plus"></i> <span
                        x-text="formVisible ? 'Ocultar formulario' : 'Agregar tarea requisito'"></span>
                </a>
            </div>
        </div>
    @else
        <div style="border: 2px solid #95CDDF;" class="rounded-3 p-3 ">
            <div class="d-flex align-items-center text-black mb-2">
                ...
                <span class="small">No hay tareas configuradas como requisito para inscribirse a este grado.</span>
            </div>
...
@script
    <script>
        $(document).ready(function() {
            ...
            $('#select-nivel-tarea-requisito').on('change', function(e) {
                @this.set('tareaSeleccionada', $(this).val());
            });

            $('#select-nivel-estado-tarea-requisito').on('change', function(e) {
            ...
            window.confirmarEliminacionNivelTareaRequisito = function(id) {
                Swal.fire({
                    title: '¿Estás seguro?',
