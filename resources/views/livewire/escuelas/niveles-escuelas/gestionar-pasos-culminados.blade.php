<div class="mb-5" x-data="{ formVisible: {{ $pasosCulminados->count() == 0 ? 'true' : 'false' }} }">
    ...
    <p class="text-dark small mb-3">Configura los pasos que se marcarán como completados cuando el usuario apruebe este
        grado</p>
    ...
    <div x-show="formVisible" x-transition class="row g-3 mb-4 align-items-end">
        <div class="col-md-5 col-sm-12" wire:ignore>
            ...
            <select id="select-nivel-paso-culminado" class="form-select select2-livewire border-1">
        ...
        <div class="col-md-5 col-sm-12" wire:ignore>
            ...
            <select id="select-nivel-estado-culminado" class="form-select select2-livewire border-1">
                <option value="">Selecciona una opción</option>
    ...
    @if ($pasosCulminados->count() > 0 || count($draftItems) > 0)
                    ...
                    <tbody>
                        @if ($draftMode)
                            @foreach ($draftItems as $index => $item)
                                <input type="hidden" name="pasos_culminados[]"
                        ...
                        @else
                            @foreach ($pasosCulminados as $index => $paso)
                                <tr>
                                    ...
                                    <td class="text-center align-middle">
                                        <button type="button"
                                            @click="confirmarEliminacionNivelPasoCulminado({{ $paso->id }})"
                                            class="btn btn-link text-danger p-0">
                                            <i class="ti ti-trash fs-5"></i>
                                        </button>
                                    </td>
            ...
            <div class="mt-3">
                <a href="#" @click.prevent="formVisible = !formVisible" style="text-decoration: underline;">
                    <i class="ti ti-circle-plus"></i> <span
                        x-text="formVisible ? 'Ocultar formulario' : 'Agregar paso a culminar'"></span>
                </a>
            </div>
        </div>
    @else
        <div style="border: 2px solid #95CDDF;" class="rounded-3 p-3 ">
            <div class="d-flex align-items-center text-black mb-2">
                ...
                <span class="small">No hay pasos configurados para marcarse al culminar el grado.</span>
            </div>
...
@script
    <script>
        $(document).ready(function() {
            ...
            $('#select-nivel-paso-culminado').on('change', function(e) {
                @this.set('pasoSeleccionado', $(this).val());
            });

            $('#select-nivel-estado-culminado').on('change', function(e) {
            ...
            window.confirmarEliminacionNivelPasoCulminado = function(id) {
                Swal.fire({
                    title: '¿Estás seguro?',
