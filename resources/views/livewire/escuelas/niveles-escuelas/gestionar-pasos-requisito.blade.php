<div class="mb-5" x-data="{ formVisible: {{ $pasosRequisito->count() == 0 ? 'true' : 'false' }} }">
    ...
    <p class="text-dark small mb-3">Configura los pasos que el usuario debe tener completados para poder inscribirse a
        este grado</p>
    ...
    <div x-show="formVisible" x-transition class="row g-3 mb-4 align-items-end">
        <div class="col-md-5 col-sm-12" wire:ignore>
            ...
            <select id="select-nivel-paso-requisito" class="form-select select2-livewire border-1">
        ...
        <div class="col-md-5 col-sm-12" wire:ignore>
            ...
            <select id="select-nivel-estado-requisito" class="form-select select2-livewire border-1">
                <option value="">Selecciona una opción</option>
    ...
    @if ($pasosRequisito->count() > 0 || count($draftItems) > 0)
                    ...
                    <tbody>
                        @if ($draftMode)
                            @foreach ($draftItems as $index => $item)
                                <input type="hidden" name="proceso_prerrequisito[]"
                        ...
                        @else
                            @foreach ($pasosRequisito as $index => $paso)
                                <tr>
                                    ...
                                    <td class="text-center align-middle">
                                        <button type="button"
                                            @click="confirmarEliminacionNivelPasoRequisito({{ $paso->id }})"
                                            class="btn btn-link text-danger p-0">
                                            <i class="ti ti-trash fs-5"></i>
                                        </button>
                                    </td>
            ...
            <div class="mt-3">
                <a href="#" @click.prevent="formVisible = !formVisible" style="text-decoration: underline;">
                    <i class="ti ti-circle-plus"></i> <span
                        x-text="formVisible ? 'Ocultar formulario' : 'Agregar paso requisito'"></span>
                </a>
            </div>
        </div>
    @else
        <div style="border: 2px solid #95CDDF;" class="rounded-3 p-3 ">
            <div class="d-flex align-items-center text-black mb-2">
                ...
                <span class="small">No hay pasos de crecimiento configurados como requisito. Los usuarios podrán
                    inscribirse sin restricciones de pasos.</span>
            </div>
...
@script
    <script>
        $(document).ready(function() {
            ...
            $('#select-nivel-paso-requisito').on('change', function(e) {
                @this.set('pasoSeleccionado', $(this).val());
            });

            $('#select-nivel-estado-requisito').on('change', function(e) {
            ...
            window.confirmarEliminacionNivelPasoRequisito = function(id) {
                Swal.fire({
                    title: '¿Estás seguro?',
