<div class="mb-5" x-data="{ formVisible: {{ $tareasCulminadas->count() == 0 ? 'true' : 'false' }} }">
    ...
    <p class="text-dark small mb-3">Configura las tareas que se marcarán como completadas cuando el usuario apruebe esta materia</p>
    ...
    <div x-show="formVisible" x-transition class="row g-3 mb-4 align-items-end">
        <div class="col-md-5 col-sm-12" wire:ignore>
            ...
            <select id="select-materia-tarea-culminada" class="form-select select2 border-1">
        ...
        <div class="col-md-5 col-sm-12" wire:ignore>
            ...
            <select id="select-materia-estado-tarea-culminada" class="form-select select2 border-1">
                <option value="">Selecciona una opción</option>
                    ...
                    <tbody>
                        @if($draftMode)
                            @foreach($draftItems as $index => $item)
                                <input type="hidden" name="tareas_culminadas[]" value="{{ $item['tarea_id'] }}|{{ $item['estado_id'] }}">
                                <tr>
                            ...
                            @foreach($tareasCulminadas as $index => $tarea)
                                <tr>
                                    ...
                                    <td class="text-center align-middle">
                                        <button
                                            type="button"
                                            @click="confirmarEliminacionMateriaTareaCulminada({{ $tarea->id }})"
                                            class="btn btn-link text-danger p-0">
                                            <i class="ti ti-trash fs-5"></i>
                                        </button>
                                    </td>
    ...
    @else
        <div style="border: 2px solid #95CDDF;" class="rounded-3 p-3 ">
            <div class="d-flex align-items-center text-black mb-2">
                ...
                <span class="small">No hay tareas configuradas para marcarse al culminar la materia.</span>
            </div>
...
<script>
    $(document).ready(function() {
        ...
        $('#select-materia-tarea-culminada').select2({
            width: '100%',
        ...
        $('#select-materia-estado-tarea-culminada').select2({
            width: '100%',
             placeholder: 'Selecciona una opción',
            allowClear: true
        }).on('change', function (e) {
        ...
        Livewire.on('tarea-agregada', () => {
             $('#select-materia-tarea-culminada').val(null).trigger('change.select2');
             $('#select-materia-estado-tarea-culminada').val(null).trigger('change.select2');
        });

        window.confirmarEliminacionMateriaTareaCulminada = function(id) {
            Swal.fire({
                title: '¿Estás seguro?',
