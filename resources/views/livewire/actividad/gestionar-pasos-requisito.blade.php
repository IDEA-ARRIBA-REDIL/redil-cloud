<div class="mb-5" x-data="{ formVisible: {{ $pasosRequisito->count() == 0 ? 'true' : 'false' }} }">
   <h5 class="fw-bold text-primary mb-1">Pasos prerequisito</h5>
    <p class="text-dark small mb-3">Configura los pasos que el usuario debe tener completados para poder inscribirse</p>
    ...
    <div x-show="formVisible" x-transition class="row g-3 mb-4 align-items-end">
        ...
        <div class="col-md-5 col-sm-12" wire:ignore>
            ...
            <select id="select-estado-requisito" class="form-select select2 border-1">
                <option value="">Selecciona una opción</option>
      ...
      {{-- Errores de validación (fuera del wire:ignore para que se actualicen) --}}
                        ...
                        @foreach($pasosRequisito as $index => $paso)
                            <tr>
                                ...
                                <td class="text-center align-middle">
                                    <button
                                        type="button"
                                        @click="confirmarEliminacionPasoRequisito({{ $paso->id }})"
...
<script>
    $(document).ready(function() {
        ...
        $('#select-estado-requisito').select2({
            width: '100%',
             placeholder: 'Selecciona una opción',
            allowClear: true
        }).on('change', function (e) {
        ...
        Livewire.on('paso-agregado', () => {
             $('#select-paso-requisito').val(null).trigger('change.select2');
             $('#select-estado-requisito').val(null).trigger('change.select2');
        });
    });

    window.confirmarEliminacionPasoRequisito = function(id) {
        Swal.fire({
            title: '¿Estás seguro?',
