<div class="mb-5" x-data="{ formVisible: {{ $tareasRequisito->count() == 0 ? 'true' : 'false' }} }">
    ...
    <p class="text-dark small mb-3">Configura las tareas que el usuario debe tener completadas para poder inscribirse</p>
    ...
    <div x-show="formVisible" x-transition class="row g-3 mb-4 align-items-end">
        <div class="col-md-5 col-sm-12" wire:ignore>
            ...
            <select id="select-tarea-requisito" class="form-select select2 border-1">
        ...
        <div class="col-md-5 col-sm-12" wire:ignore>
            ...
            <select id="select-estado-tarea-requisito" class="form-select select2 border-1">
                <option value="">Selecciona una opción</option>
                        ...
                        @foreach($tareasRequisito as $index => $tarea)
                            <tr>
                                ...
                                <td class="text-center align-middle">
                                    <button
                                        type="button"
                                        @click="confirmarEliminacionTareaRequisito({{ $tarea->id }})"
    ...
    @else
        <div style="border: 2px solid #95CDDF;" class="rounded-3 p-3 ">
             <div class="d-flex align-items-center text-black mb-2">
                ...
                <span class="small">No hay tareas de consolidación configuradas como requisito. Los usuarios podrán inscribirse sin restricciones de tareas.</span>
            </div>
...
<script>
    $(document).ready(function() {
        ...
        $('#select-tarea-requisito').select2({
            width: '100%',
        ...
        $('#select-estado-tarea-requisito').select2({
            width: '100%',
             placeholder: 'Selecciona una opción',
            allowClear: true
        }).on('change', function (e) {
        ...
        Livewire.on('tarea-agregada', () => {
             $('#select-estado-tarea-requisito').val(null).trigger('change.select2');
        });

        window.confirmarEliminacionTareaRequisito = function(id) {
            Swal.fire({
                title: '¿Estás seguro?',
