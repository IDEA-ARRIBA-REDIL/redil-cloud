<div>
    @if($posts->isNotEmpty())
        ...
        <li class="nav-item me-3 d-flex align-items-center">
            <a class="nav-link p-0 position-relative cursor-pointer" onclick="openNavbarPostModal(0)" title="Nuevas publicaciones">
                <div class="avatar avatar-md border border-2 rounded-circle" style="border-color: #28c76f !important; padding: 2px;">
                    @php
        ...
        <!-- Modal Inmersivo Navbar (Teletransportado al Body) -->
        @teleport('body')
        <div class="modal fade border-0" id="modalNavbarPostsInmersivo" tabindex="-1" aria-hidden="true" wire:ignore.self style="background: rgba(0,0,0,0.6) !important;">
                ...
                <div class="modal-content bg-transparent border-0 shadow-none h-100">
                    <div class="modal-body p-0 h-100 position-relative d-flex justify-content-center align-items-center">
                        ...
                        <div class="w-100 shadow-lg position-relative" style="max-width: 450px; height: 90vh; max-height: 850px; background: #000; border-radius: 15px; overflow: hidden;">
                            ...
                            <!-- Indicador Desliza -->
                            <div id="indicador-desliza-navbar" class="position-absolute top-50 start-50 text-white text-center transition-all duration-500" style="z-index: 100; opacity: 0.7; pointer-events: none; animation: bounce-vertical-nav 2s infinite; transition: opacity 0.5s ease-out; text-shadow: 0 2px 10px rgba(0,0,0,0.8); transform: translate(-50%, -50%);">
                            ...
                            <div class="swiper-container swiper swiper-posts-vertical-navbar w-100 h-100" id="swiper-posts-vertical-navbar">
                                <div class="swiper-wrapper h-100">
                                    @foreach($posts as $index => $post)
                                        <div class="swiper-slide list-post-item-vertical h-100 w-100 position-relative" wire:key="navbar-modal-post-{{ $post->id }}">
                                            ...
                                            <!-- Fondo -->
                                            @if($post->image_path)
                                                <img src="{{ Storage::url($configuracion->ruta_almacenamiento.'/img/publicaciones/'.$post->image_path) }}" alt="Publicación" class="w-100 h-100" style="object-fit: cover; object-position: center;">
                                            @else
                                                @php
                                                    $selectedGradient = $gradients[$post->id % count($gradients)];
                                                @endphp
                                                <div id="nav-modal-capture-{{ $post->id }}" class="w-100 h-100 d-flex align-items-center justify-content-center p-4 text-center" style="background: {{ $selectedGradient }};">
                                            ...
                                            <div class="position-absolute bottom-0 end-0 mb-4 me-3 d-flex flex-column align-items-center gap-4" style="z-index: 20;">
                                                <div class="d-flex flex-column align-items-center"
                                                    ...
                                                    <div class="rounded-circle d-flex align-items-center justify-content-center cursor-pointer mb-1 shadow" 
                                                        ...
                                                        <i class="ti" style="font-size: 1.8rem;" :class="liked ? 'ti-heart-filled text-danger' : 'ti-heart text-white'"></i>
                                                    </div>
            ...
            function resetearIndicadorNav() {
                ...
                if(indicador) {
                    ...
                    window.timerIndicadorNav = setTimeout(() => {
                        ...
                        setTimeout(() => { indicador.classList.add('d-none'); }, 500);
                    }, 4000);
                }
            }
            ...
            if (window.Livewire) {
                Livewire.hook('commit', ({ component, succeed }) => {
                    if (component.name === 'navbar-posts-status') {
                        succeed(() => {
                            setTimeout(() => {
                                if(document.getElementById('modalNavbarPostsInmersivo') && document.getElementById('modalNavbarPostsInmersivo').classList.contains('show')) {
                                    initNavbarVerticalSwiper();
                                }
