@if($versiculo)
<div class="{{ $claseColumnas }}" wire:key="versiculo-dashboard-{{ $versiculo->id }}">
    @if($versiculo)
        @php
            $relativeUrl = $versiculo->ruta_imagen ? Storage::url($configuracion->ruta_almacenamiento.'/img/versiculo-diario/'.$versiculo->ruta_imagen) : '';
            $imageUrl = $relativeUrl ? (str_starts_with($relativeUrl, 'http') ? $relativeUrl : config('app.url') . $relativeUrl) : url()->current();
        @endphp
        <div class="card h-100 shadow-sm border-0 overflow-hidden position-relative" style="border-radius: 15px;">
            ...
            <div class="card-img-top position-relative overflow-hidden" style="width: 100%; height: 0; padding-bottom: 100%; background-color: #f8f9fa;">
                ...
                @else
                    ...
                    @endphp
                    <div id="capture-container-{{ $versiculo->id }}" class="position-absolute top-0 start-0 w-100 h-100 d-flex align-items-center justify-content-center p-4 text-center" style="background: {{ $selectedGradient }};" wire:ignore>
                        <div class="text-white">
                            <i class="ti ti-bible mb-2 d-block opacity-75" style="font-size: 2.5rem;"></i>
                            <p class="mb-0 fw-medium" style="font-size: 1rem; line-height: 1.5; display: -webkit-box; -webkit-line-clamp: 6; -webkit-box-orient: vertical; overflow: hidden;">
                                {{ $plainText ?: $versiculo->cita_referencia }}
                            </p>
                            <small class="mt-2 d-block" style="font-size: 0.7rem;">{{ $versiculo->cita_referencia }}</small>
                        </div>
                    </div>
                @endif
            </div>
                    ...
                    <div class="d-flex">
                        <div class="d-flex align-items-center gap-2" wire:ignore>
                            @if($versiculo->url_video_reflexion)
                                <button type="button" 
                                        ...
                                        data-bs-target="#modalVideoDashboard" 
                        ...
                        <div class="d-flex align-items-center gap-3 text-black">
                            ...
                            <div class="d-flex align-items-center gap-1">
                                <i class="ti {{ $versiculo->usuariosQueDieronLike()->where('usuario_id', auth()->id())->exists() ? 'ti-heart-filled text-danger' : 'ti-heart' }} ti-sm cursor-pointer" 
                            ...
                            <div id="containerAccionVersiculo" wire:ignore>
                                <i class="ti ti-share ti-sm cursor-pointer text-black d-none" 
                                   id="btnShareMobile"
                                ...
                                <i class="ti ti-download ti-sm cursor-pointer text-black d-none" 
                                   id="btnDownloadPC"
        ...
        @once
        <div class="modal fade modal-transparent" id="modalVideoDashboard" tabindex="-1" aria-hidden="true" wire:ignore.self>
            <div class="modal-dialog modal-dialog-centered">
                <div class="modal-content">
                    ...
                    <div class="modal-body px-5 position-relative">
                        <div id="ratioContainerDashboard" class="ratio ratio-16x9 bg-black rounded-3 overflow-hidden shadow-lg border border-secondary border-opacity-25">
                            <iframe id="videoIframeDashboard" src="" title="Video reflexión" allowfullscreen allow="autoplay; encrypted-media"></iframe>
                        </div>
                    </div>
                </div>
            </div>
            ...
            <div class="modal-dialog modal-dialog-centered modal-lg" role="document">
                <div class="modal-content shadow-lg border-0">
                    ...
                    <div class="modal-body text-center pt-0 pb-5 px-4 overflow-auto" style="max-height: 80vh;">
                        ...
                        <div id="modalVersiculoTextoDashboard" class="fs-6 px-md-5 text-black text-start" style="line-height: 1.6; font-style: italic;">
        ...
        <style>
            ...
            .modal-transparent .modal-content { background: transparent !important; border: 0 !important; box-shadow: none !important; }
            .btn-close-video { z-index: 2000 !important; filter: invert(1) grayscale(100%) brightness(200%); background-color: rgba(0,0,0,0.5); border-radius: 50%; padding: 10px; }
        ...
        <script>
            ...
            async function handleShareClick(e, cita, urlImagen, texto, captureId) {
                const shareData = { title: 'Versículo del Día: ' + cita, text: '\"' + texto + '\" - ' + cita, url: '{{ url()->current() }}' };
            ...
            document.addEventListener('DOMContentLoaded', function () {
                ...
                document.addEventListener('show.bs.modal', function (event) {
                    ...
                    if (targetId === 'modalVideoDashboard') {
                        ...
                        let isVertical = url.includes('shorts') || url.includes('reels') || url.includes('tiktok.com');

                        if (url.includes('youtube.com/watch?v=')) url = url.replace('watch?v=', 'embed/');
                        else if (url.includes('youtu.be/')) url = `https://www.youtube.com/embed/${url.split('/').pop().split('?')[0]}`;
                        else if (url.includes('youtube.com/shorts/')) { url = url.replace('shorts/', 'embed/'); isVertical = true; }
                        else if (url.includes('vimeo.com/')) url = `https://player.vimeo.com/video/${url.split('/').pop().split('?')[0]}`;
