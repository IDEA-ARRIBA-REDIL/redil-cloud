<div class="{{ $claseColumnas }}">
    @if($posts->isNotEmpty())
        ...
        <div class="swiper-container swiper swiper-posts position-relative" id="swiper-posts-widget" wire:ignore.self style="min-height: 250px;">
            <!-- Skeleton Loader (se oculta cuando Swiper se inicializa) -->
            <div id="posts-skeleton" class="d-flex gap-2 overflow-hidden position-absolute top-0 start-0 w-100 h-100" style="z-index: 10; background: #fff;">
                @for ($i = 0; $i < 5; $i++)
                    <div class="flex-shrink-0" style="width: calc(20% - 10px); height: 100%; border-radius: 12px; background: linear-gradient(90deg, #f0f0f0 25%, #e0e0e0 50%, #f0f0f0 75%); background-size: 200% 100%; animation: skeleton-loading 1.5s infinite;"></div>
                @endfor
            </div>
            <div class="swiper-wrapper">
                @foreach($posts as $index => $post)
                    <div class="swiper-slide list-post-item" wire:key="post-{{ $post->id }}" wire:ignore.self>
                        ...
                        <div class="card h-100 border-0 overflow-hidden position-relative shadow-none cursor-pointer" style="border-radius: 12px; background: #000;" onclick="openPostModal({{ $index }})">
                            <!-- Top Image/Text -->
                            <div class="card-img-top position-relative overflow-hidden" style="width: 100%; height: 0; padding-bottom: 177.77%;">
                                @if($post->image_path)
                                    @php
                                ...
                                @else
                                    ...
                                    @endphp
                                    <div id="capture-post-{{ $post->id }}" class="position-absolute top-0 start-0 w-100 h-100 d-flex align-items-center justify-content-center p-3 text-center" style="background: {{ $selectedGradient }};">
                                        <div class="text-white">
                                            <p class="mb-0 fw-medium" style="font-size: 0.9rem; line-height: 1.4; display: -webkit-box; -webkit-line-clamp: 6; -webkit-box-orient: vertical; overflow: hidden;">
                                                {!! $post->descripcion !!}
                                            </p>
                                        </div>
                                    </div>
                                ...
                                <div class="position-absolute top-0 start-0 w-100 h-100" style="background: linear-gradient(to bottom, rgba(0,0,0,0) 60%, rgba(0,0,0,0.4) 100%);"></div>

                                <!-- Botón Like (Centrado a la derecha) -->
                                <div class="position-absolute end-0 top-50 translate-middle-y me-2 d-flex flex-column align-items-center" style="z-index: 5;"
                                     x-data="{ 
                                         postId: {{ $post->id }},
                                    ...
                                    <span class="text-white fw-bold" style="font-size: 0.75rem; text-shadow: 0 1px 2px rgba(0,0,0,0.5);" x-text="count"></span>
                                </div>

                                <!-- Botón Compartir / Descargar (Esquina inferior derecha) -->
                                <div class="position-absolute bottom-0 end-0 mb-2 me-2" style="z-index: 5;">
                                    @php
                                        $postImageUrl = $post->image_path ? Storage::url($configuracion->ruta_almacenamiento.'/img/publicaciones/'.$post->image_path) : '';
                                        $postImageUrl = $postImageUrl ? (str_starts_with($postImageUrl, 'http') ? $postImageUrl : config('app.url') . $postImageUrl) : url()->current();
        ...
        <div class="modal fade" id="modalPostsInmersivo" tabindex="-1" aria-hidden="true" wire:ignore.self>
            <div class="modal-dialog modal-dialog-centered modal-fullscreen-md-down modal-lg h-100 my-0 py-3">
                <div class="modal-content bg-transparent border-0 shadow-none h-100">
                    <div class="modal-body p-0 h-100 position-relative d-flex justify-content-center align-items-center">
                        <!-- Contenedor con aspecto vertical máximo -->
                        <div class="w-100 shadow-lg position-relative" style="max-width: 450px; height: 90vh; max-height: 850px; background: #000; border-radius: 15px; overflow: hidden;">
                            
                            <!-- Indicador Desliza -->
                            <div id="indicador-desliza" class="position-absolute top-50 start-50 text-white text-center transition-all duration-500" style="z-index: 100; opacity: 0.7; pointer-events: none; animation: bounce-vertical 2s infinite; transition: opacity 0.5s ease-out; text-shadow: 0 2px 10px rgba(0,0,0,0.8); transform: translate(-50%, -50%);">
                            ...
                            <!-- Swiper Vertical -->
                            <div class="swiper-container swiper swiper-posts-vertical w-100 h-100" id="swiper-posts-vertical">
                                <div class="swiper-wrapper h-100">
                                    @foreach($posts as $index => $post)
                                        <div class="swiper-slide list-post-item-vertical h-100 w-100 position-relative" wire:key="modal-post-{{ $post->id }}">
                                            
                                            <!-- Top Header: User Info y Botón Cerrar -->
                                            <div class="position-absolute top-0 start-0 w-100 p-3 pt-4 d-flex align-items-center justify-content-between" style="z-index: 50; background: linear-gradient(to bottom, rgba(0,0,0,0.6) 0%, rgba(0,0,0,0) 100%);">
                                                
                                                <div class="d-flex align-items-center">
                                                    @if($post->user && $post->user->profile_photo_path)
                                                        <img src="{{ Storage::url($post->user->profile_photo_path) }}" class="rounded-circle me-3 border border-white" width="45" height="45" style="object-fit: cover;">
                                                    @else
                                                        <div class="rounded-circle bg-primary text-white d-flex align-items-center justify-content-center me-3 border border-white fw-bold fs-5" style="width: 45px; height: 45px;">
                                                            {{ strtoupper(substr($post->user->name ?? 'U', 0, 1)) }}
                                                        </div>
                                                    @endif
                                                    <h6 class="text-white mb-0 fw-bold" style="text-shadow: 0 1px 3px rgba(0,0,0,0.8);">{{ $post->user->name ?? 'Usuario' }}</h6>
                                                </div>
                                                <button type="button" class="btn-close btn-close-white ms-auto mt-5 me-5" data-bs-dismiss="modal" aria-label="Close" style="background-color: rgba(0,0,0,0.5); border-radius: 50%; padding: 10px;"></button>
                                            </div>
                                            ...
                                            @if($post->image_path)
                                                @php
                                                    $relativeUrl = Storage::url($configuracion->ruta_almacenamiento.'/img/publicaciones/'.$post->image_path);
                                                @endphp
                                                <img src="{{ $relativeUrl }}" alt="Publicación" class="w-100 h-100" style="object-fit: cover; object-position: center;">
                                            @else
                                                @php
                                                    $gradients = [
                                                        'linear-gradient(135deg, #7367f0 0%, #a8a1f3 100%)',
                                                ...
                                                @endphp
                                                <div id="modal-capture-post-{{ $post->id }}" class="w-100 h-100 d-flex align-items-center justify-content-center p-4 text-center" style="background: {{ $selectedGradient }};">
                                                    <div class="text-white">
                                                        <p class="mb-0 fw-medium" style="font-size: 1.2rem; line-height: 1.5; text-shadow: 0 1px 3px rgba(0,0,0,0.5);">
                                                            {!! $post->descripcion !!}
                                                        </p>
                                                    </div>
                                                </div>
                                            ...
                                            <div class="position-absolute bottom-0 start-0 w-100 p-4" style="z-index: 10; background: linear-gradient(to top, rgba(0,0,0,0.9) 0%, rgba(0,0,0,0.5) 50%, rgba(0,0,0,0) 100%); min-height: 40%;">
                                                <!-- Descripción -->
                                                <div class="text-white me-5 pe-3 position-absolute bottom-0 mb-4" style="max-height: 30vh; overflow-y: auto;">
                                                    <p class="mb-0 lh-base" style="font-size: 0.95rem; text-shadow: 0 1px 3px rgba(0,0,0,0.9);">
                                            ...
                                            <div class="position-absolute bottom-0 end-0 mb-4 me-3 d-flex flex-column align-items-center gap-4" style="z-index: 20;">
                                                <!-- Botón Like -->
                                                <div class="d-flex flex-column align-items-center"
                                                    ...
                                                    <div class="rounded-circle d-flex align-items-center justify-content-center cursor-pointer mb-1 shadow" 
                                                        ...
                                                        <i class="ti" 
                                                           ...
                                                           :class="liked ? 'ti-heart-filled text-danger' : 'ti-heart text-white'"></i>
                                                    </div>
                                                    <span class="text-white fw-bold" style="font-size: 0.85rem; text-shadow: 0 1px 3px rgba(0,0,0,0.9);" x-text="count"></span>
                                                </div>
                                                ...
                                                <div class="d-flex flex-column align-items-center">
                                                    <div class="rounded-circle d-flex align-items-center justify-content-center cursor-pointer btn-share-post d-none mb-1 shadow" 
                                                        ...
                                                        <i class="ti ti-share text-white" style="font-size: 1.8rem;"></i>
                                                    </div>
                                                    ...
                                                    <div class="rounded-circle d-flex align-items-center justify-content-center cursor-pointer btn-download-post d-none mb-1 shadow" 
                                                        ...
                                                        <i class="ti ti-download text-white" style="font-size: 1.8rem;"></i>
                                                    </div>
                                                </div>
                                            </div>
        ...
        <script>
            ...
            async function handleSharePost(e, texto, urlImagen, captureId) {
                const shareData = { title: 'Publicación', text: '\"' + texto + '\"', url: '{{ url()->current() }}' };
                ...
                if (navigator.share) {
                    try {
                        ...
                        if (urlImagen) {
                            ...
                            const canvas = await html2canvas(document.getElementById(captureId), { scale: 2 });
        ...
        <script>
            ...
            function initPostsSwiper() {
                ...
                postsSwiper = new Swiper(container, {
                    ...
                    breakpoints: {
                        ...
                        1200: { slidesPerView: 5.2, spaceBetween: 10 }
                    },
                    on: {
                        reachEnd: function () {
            ...
            function ocultarIndicadorDesliza() {
                ...
                if(indicador && indicador.style.opacity !== '0') {
                    ...
                    setTimeout(() => { indicador.classList.add('d-none'); }, 500); // 500ms tras la transicion
                ...
                Livewire.hook('commit', ({ component, succeed }) => {
                    if (component.name === 'dashboard.posts-widget') {
                        succeed(() => {
                            setTimeout(() => {
                                initPostsSwiper();
                                if(document.getElementById('modalPostsInmersivo').classList.contains('show')) {
                                    initVerticalSwiper();
                                }
                                evaluarBotones();
                            }, 50);
