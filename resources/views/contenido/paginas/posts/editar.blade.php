    ...
    $(function () {
        ...
        if (flatpickrDateInicio) {
            flatpickrDateInicio.flatpickr({
                ...
                defaultDate: '{{ $post->fecha_inicio->format('Y-m-d') }}'
            });
        }
        ...
        if (flatpickrDateFin) {
            flatpickrDateFin.flatpickr({
                monthSelectorType: 'static',
                defaultDate: '{{ $post->fecha_fin ? $post->fecha_fin->format('Y-m-d') : '' }}'
            });
        }
        ...
        $.getScript('https://cdn.jsdelivr.net/npm/quill-emoji@0.2.0/dist/quill-emoji.js', function() {
            ...
            quillEditor.root.innerHTML = `{!! $post->descripcion !!}`;
        ...
        $('#formulario').submit(function(){
            ...
            Swal.fire({
                ...
                text: "Ya estamos actualizando la publicación...",
                icon: "info",
        ...
        // Inicializar estado de fecha fin
        if ($('#visualizar_siempre').is(':checked')) {
        ...
        // Estado inicial de restricciones (Sincronizar con el valor actual)
        toggleRestricciones();

        let contadorPasos = {{ count($pasosPost) }};
        let contadorTareas = {{ count($tareasPost) }};
        ...
        $(document).on('click', '.btn-eliminar-fila', function() {
            $(this).closest('.fila-paso, .fila-tarea').remove();
        });
    });
</script>
@endsection
...
<h4 class="mb-1 fw-semibold text-primary">Editar publicación</h4>

@include('layouts.status-msn')

<form action="{{ route('posts.update', $post) }}" method="POST" enctype="multipart/form-data" id="formulario">
  @csrf
  @method('PATCH')
  <div class="row mt-10">
    <div class="col-md-4">
            ...
            <div class="row">
              <div class="mb-3 col-md-12 d-flex justify-content-center align-items-center flex-column">
                  <div class="position-relative d-inline-block">
                      ...
                      <button type="button" class="btn btn-sm btn-icon btn-primary rounded-circle position-absolute bottom-0 end-0 mb-n2 me-n2 shadow" data-bs-toggle="modal" data-bs-target="#modalFoto">
            ...
            <div class="row">
              <div class="mb-3 col-md-12">
               ...
               <div class="mb-3 col-12 col-md-12">
                <div class="form-check form-switch pt-2">
                    <input class="form-check-input" type="checkbox" id="visualizar_siempre" name="visualizar_siempre" value="1" {{ $post->visualizar_siempre ? 'checked' : '' }}>
                    <label class="form-check-label w-100 cursor-pointer" for="visualizar_siempre">
                        <strong>Visualizar siempre</strong>
                        <small class="d-block text-black">Si se activa, la publicación no tendrá fecha de vencimiento.</small>
                    </label>
                </div>
              </div>
              ...
              <div class="mb-3 col-md-6 col-12">
                ...
                <div class="input-group">
                  ...
                  <input type="text" class="form-control" id="fecha_inicio" name="fecha_inicio" placeholder="YYYY-MM-DD" required />
                </div>
              </div>
              ...
              <div class="mb-3 col-md-6 col-12" id="fecha_fin_container">
                ...
                <div class="input-group">
                  <span class="input-group-text"><i class="ti ti-calendar"></i></span>
                  <input type="text" class="form-control" id="fecha_fin" name="fecha_fin" placeholder="YYYY-MM-DD" />
                </div>
              </div>
  ...
  <div class="row g-3">
    <div class="col-12 mt-4">
        <div class="card border shadow-none mb-0">
            <div class="card-header border-bottom py-2 cursor-pointer d-flex justify-content-between align-items-center" data-bs-toggle="collapse" data-bs-target="#collapseRestricciones" aria-expanded="{{ $post->visible_todos ? 'false' : 'true' }}" aria-controls="collapseRestricciones">
                <h5 class="card-header text-black fw-semibold">
                    Restricciones de visibilidad
                </h5> 
                <i class="ti ti-chevron-down"></i>
            </div>
            <div id="collapseRestricciones" class="collapse {{ $post->visible_todos ? '' : 'show' }}">
                <div class="card-body pt-3">
                    <div class="row g-3"> 
                        <!-- Visible para todos -->
                        <div class="col-12 mb-2"> 
                            <div class="form-check form-switch">
                                <input class="form-check-input" type="checkbox" id="visible_todos" name="visible_todos" {{ $post->visible_todos ? 'checked' : '' }}>
                        ...
                        <div id="seccionRestricciones" style="display: {{ $post->visible_todos ? 'none' : 'block' }};">
                            <hr class="my-3">
                            <div class="row g-3">
                                <!-- Género -->
                                <div class="col-12">
                                    ...
                                    <div class="row">
                                        <div class="col-4">
                                            <div class="form-check custom-option custom-option-icon">
                                                <label class="form-check-label custom-option-content" for="genero_ambos">
                                                    <span class="custom-option-body">
                                                        <span class="custom-option-title">Ambos</span>
                                                    </span>
                                                    <input name="genero" class="form-check-input" type="radio" value="3" id="genero_ambos" {{ $post->genero == 3 ? 'checked' : '' }} />
                                                </label>
                                            </div>
                                        </div>
                                        <div class="col-4">
                                            <div class="form-check custom-option custom-option-icon">
                                                <label class="form-check-label custom-option-content" for="genero_masculino">
                                                    <span class="custom-option-body">
                                                        <span class="custom-option-title">Hombre</span>
                                                    </span>
                                                    <input name="genero" class="form-check-input" type="radio" value="1" id="genero_masculino" {{ $post->genero == 1 ? 'checked' : '' }} />
                                                </label>
                                            </div>
                                        </div>
                                        <div class="col-4">
                                            <div class="form-check custom-option custom-option-icon">
                                                <label class="form-check-label custom-option-content" for="genero_femenino">
                                                    <span class="custom-option-body">
                                                        <span class="custom-option-title">Mujer</span>
                                                    </span>
                                                    <input name="genero" class="form-check-input" type="radio" value="2" id="genero_femenino" {{ $post->genero == 2 ? 'checked' : '' }} />
                                                </label>
                                            </div>
                                        </div>
                                    </div>
                                ...
                                <div class="col-12">
                                    ...
                                    <select id="sedes" name="sedes[]" class="select2 form-select" multiple>
                                        @foreach($sedes as $sede)
                                            <option value="{{ $sede->id }}" {{ in_array($sede->id, $sedesPost) ? 'selected' : '' }}>{{ $sede->nombre }}</option>
                                        @endforeach
                                    </select>
                                </div>
                                ...
                                <div class="col-12">
                                    ...
                                    <select id="estadosCiviles" name="estadosCiviles[]" class="select2 form-select" multiple>
                                        @foreach($estadosCiviles as $estado)
                                            <option value="{{ $estado->id }}" {{ in_array($estado->id, $estadosCivilesPost) ? 'selected' : '' }}>{{ $estado->nombre }}</option>
                                        @endforeach
                                    </select>
                                </div>
                                ...
                                <div class="col-12">
                                    ...
                                    <select id="rangosEdad" name="rangosEdad[]" class="select2 form-select" multiple>
                                        @foreach($rangosEdad as $rango)
                                            <option value="{{ $rango->id }}" {{ in_array($rango->id, $rangosEdadPost) ? 'selected' : '' }}>{{ $rango->nombre }} ({{ $rango->edad_minima }}-{{ $rango->edad_maxima }})</option>
                                        @endforeach
                                    </select>
                                </div>
                                ...
                                <div class="col-12">
                                    ...
                                    <select id="tiposUsuario" name="tiposUsuario[]" class="select2 form-select" multiple>
                                        @foreach($tiposUsuario as $tipo)
                                            <option value="{{ $tipo->id }}" {{ in_array($tipo->id, $tiposUsuarioPost) ? 'selected' : '' }}>{{ $tipo->nombre }}</option>
                                        @endforeach
                                    </select>
                                </div>
                                                ...
                                                <div class="col-md-6">
                                                    <select name="pasos[{{ $index }}][id]" class="form-select select-paso" required>
                                                        ...
                                                        @foreach($pasosCrecimiento as $paso)
                                                            <option value="{{ $paso->id }}" {{ $pp->id == $paso->id ? 'selected' : '' }}>{{ $paso->nombre }}</option>
                                                        @endforeach
                                                    </select>
                                                </div>
                                                <div class="col-md-5">
                                                    <select name="pasos[{{ $index }}][estado]" class="form-select select-estado-paso" required>
                                                        ...
                                                        @foreach($estadosPasos as $estado)
                                                            <option value="{{ $estado->id }}" {{ $pp->pivot->estado_paso_crecimiento_usuario_id == $estado->id ? 'selected' : '' }}>{{ $estado->nombre }}</option>
                                                        @endforeach
                                                    </select>
                                                </div>
                                ...
                                <div class="col-12 mt-4">
                                    ...
                                    <div id="contenedorTareas">
                                        @foreach($tareasPost as $index => $tp)
                                                ...
                                                <div class="col-md-6">
                                                    <select name="tareas[{{ $index }}][id]" class="form-select select-tarea" required>
                                                        ...
                                                        @foreach($tareasConsolidacion as $tarea)
                                                            <option value="{{ $tarea->id }}" {{ $tp->id == $tarea->id ? 'selected' : '' }}>{{ $tarea->nombre }}</option>
                                                        @endforeach
                                                    </select>
                                                </div>
                                                <div class="col-md-5">
                                                    <select name="tareas[{{ $index }}][estado]" class="form-select select-estado-tarea" required>
                                                        ...
                                                        @foreach($estadosTareas as $estado)
                                                            <option value="{{ $estado->id }}" {{ $tp->pivot->estado_tarea_consolidacion_id == $estado->id ? 'selected' : '' }}>{{ $estado->nombre }}</option>
                                                        @endforeach
                                                    </select>
                                                </div>
                                                <div class="col-md-1 text-end">
    ...
    <div class="modal-content">
      <div class="modal-body p-0">
        ...
        <div class="row px-4">
          <div class="col-12">
            ...
            <div class="mb-3">
              <label class="form-label fw-bold d-block text-center mt-4 mb-2">2. Recorta la foto (9:16)</label>
              <div class="cropper-container" style="max-height: 400px; overflow: hidden;">
                <img src="{{ $post->image_path ? asset('storage/'.$configuracion->ruta_almacenamiento.'/img/publicaciones/'.$post->image_path) : Storage::url('generales/img/others/placeholder.jpg') }}" class="w-100" id="croppingImage" alt="cropper">
              </div>
            </div>
          </div>
        </div>
      ...
      <div class="modal-footer border-0 p-4">
        <div class="col-12 text-center">
          <button type="button" class="btn btn-primary crop me-sm-3 me-1 px-5" data-bs-dismiss="modal">Recortar y Guardar</button>
          <button type="button" class="btn btn-label-secondary px-5" data-bs-dismiss="modal" aria-label="Close">Cerrar</button>
        </div>
      </div>
    </div>
  </div>
