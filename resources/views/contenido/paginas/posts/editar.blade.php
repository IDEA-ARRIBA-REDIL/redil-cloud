                            ...
                            <div class="mb-3 col-12 col-md-12">
                                <div class="form-check form-switch pt-2">
                                    <input class="form-check-input" type="checkbox" id="visualizar_siempre"
                                        ...
                                        {{ $post->visualizar_siempre ? 'checked' : '' }}>
            ...
            <div class="col-12 mt-4">
                <div class="card border shadow-none mb-0">
                    <div class="card-header border-bottom py-2 cursor-pointer d-flex justify-content-between align-items-center"
                        data-bs-toggle="collapse" data-bs-target="#collapseRestricciones"
                        aria-expanded="{{ $post->visible_todos ? 'false' : 'true' }}" aria-controls="collapseRestricciones">
                        <h5 class="card-header text-black fw-semibold">
                                ...
                                <div class="col-12 mb-2">
                                    <div class="form-check form-switch">
                                        <input class="form-check-input" type="checkbox" id="visible_todos"
                                            name="visible_todos" {{ $post->visible_todos ? 'checked' : '' }}>
                                        <label class="form-check-label w-100 cursor-pointer" for="visible_todos">
                                            ...
                                            <small class="d-block text-muted">Si se desactiva, solo los usuarios que cumplan
                                                los requisitos podrán verla.</small>
                                        </label>
                                    </div>
                                </div>

                                <div id="seccionRestricciones"
                                    style="display: {{ $post->visible_todos ? 'none' : 'block' }};">
                                    <hr class="my-3">
                                    <div class="row g-3">
                                        <!-- Género -->
                                        <div class="col-12">
                                                ...
                                                <div class="col-md-4">
                                                    <div class="form-check custom-option custom-option-basic">
                                                        <label
                                                            ...
                                                            <input name="genero" class="form-check-input" type="radio"
                                                                value="3" id="genero_ambos"
                                                                {{ $post->genero == 3 ? 'checked' : '' }} />
                                                        </label>
                                                    </div>
                                                </div>
                                            </div>
                                            ...
                                            <select id="sedes" name="sedes[]" class="select2 form-select" multiple>
                                                @foreach ($sedes as $sede)
                                                    <option value="{{ $sede->id }}"
                                                        {{ in_array($sede->id, $sedesPost) ? 'selected' : '' }}>
                                                        {{ $sede->nombre }}</option>
                                                @endforeach
                                            </select>
                                        </div>
                                            ...
                                            <select id="estadosCiviles" name="estadosCiviles[]"
                                                ...
                                                @foreach ($estadosCiviles as $estado)
                                                    <option value="{{ $estado->id }}"
                                                        {{ in_array($estado->id, $estadosCivilesPost) ? 'selected' : '' }}>
                                            ...
                                            <select id="rangosEdad" name="rangosEdad[]" class="select2 form-select"
                                                ...
                                                @foreach ($rangosEdad as $rango)
                                                    <option value="{{ $rango->id }}"
                                                        {{ in_array($rango->id, $rangosEdadPost) ? 'selected' : '' }}>
                                                        {{ $rango->nombre }}
                                            ...
                                            <select id="tiposUsuario" name="tiposUsuario[]" class="select2 form-select"
                                                ...
                                                @foreach ($tiposUsuario as $tipo)
                                                    <option value="{{ $tipo->id }}"
                                                        {{ in_array($tipo->id, $tiposUsuarioPost) ? 'selected' : '' }}>
                                                        {{ $tipo->nombre }}</option>
                                                @endforeach
                                            </select>
                                        </div>
                                            ...
                                            <div id="contenedorPasos">
                                                @foreach ($pasosPost as $index => $pp)
                                                    <div class="row g-2 mt-1 align-items-center fila-paso">
                                                        <div class="col-md-6">
                                                            <select name="pasos[{{ $index }}][id]"
                                                                class="form-select select-paso" required>
                                                                <option value="">Seleccionar Paso...</option>
                                                                @foreach ($pasosCrecimiento as $paso)
                                                                    <option value="{{ $paso->id }}"
                                                                        {{ $pp->id == $paso->id ? 'selected' : '' }}>
                                                                        {{ $paso->nombre }}</option>
                                                                @endforeach
                                                            </select>
                                                        </div>
                                                        <div class="col-md-5">
                                                            <select name="pasos[{{ $index }}][estado]"
                                                                class="form-select select-estado-paso" required>
                                                                <option value="">Estado Requerido...</option>
                                                                @foreach ($estadosPasos as $estado)
                                                                    <option value="{{ $estado->id }}"
                                                                        {{ $pp->pivot->estado_paso_crecimiento_usuario_id == $estado->id ? 'selected' : '' }}>
                                                                        {{ $estado->nombre }}</option>
                                                                @endforeach
                                                            </select>
                                                        </div>
                                                        <div class="col-md-1 text-end">
                                                            <button type="button"
                                                                class="btn btn-link text-danger p-0 btn-eliminar-fila">
                                            ...
                                            <div id="contenedorTareas">
                                                @foreach ($tareasPost as $index => $tp)
                                                    <div class="row g-2 mt-1 align-items-center fila-tarea">
                                                        <div class="col-md-6">
                                                            <select name="tareas[{{ $index }}][id]"
                                                                class="form-select select-tarea" required>
                                                                <option value="">Seleccionar Tarea...</option>
                                                                @foreach ($tareasConsolidacion as $tarea)
                                                                    <option value="{{ $tarea->id }}"
                                                                        {{ $tp->id == $tarea->id ? 'selected' : '' }}>
                                                                        {{ $tarea->nombre }}</option>
                                                                @endforeach
                                                            </select>
                                                        </div>
                                                        <div class="col-md-5">
                                                            <select name="tareas[{{ $index }}][estado]"
                                                                class="form-select select-estado-tarea" required>
                                                                <option value="">Estado Requerido...</option>
                                                                @foreach ($estadosTareas as $estado)
                                                                    <option value="{{ $estado->id }}"
                                                                        {{ $tp->pivot->estado_tarea_consolidacion_id == $estado->id ? 'selected' : '' }}>
                                                                        {{ $estado->nombre }}</option>
                                                                @endforeach
                                                            </select>
                                                        </div>
