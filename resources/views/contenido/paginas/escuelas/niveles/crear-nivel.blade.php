...
{{-- Título de la página --}}
@section('title', 'Crear Nuevo Nivel - Escuela: ' . $escuela->nombre)
...
<script type="module">
  ...
  editorNivel.root.innerHTML = `{!! old('descripcion') !!}`;
...
<form id="formNuevoNivel" action="{{ route('niveles.guardar', $escuela) }}" method="POST" enctype="multipart/form-data">
  @csrf
  ...
  <div class="col-md-12">
    <div class="card mb-4 rounded rounded-3">
      <img id="preview-foto" class="cropped-img card-img-top mb-2"
        src="{{ Storage::url($configuracion->ruta_almacenamiento . '/img/niveles/default.png') }}"
        {{-- Cambia a ruta por defecto para niveles --}} alt="Portada Nivel {{ $escuela->nombre }}"
      ...
      <div class="row p-4 m-0 d-flex card-body">
        <h5 class="mb-1 fw-semibold text-black">Crear Nivel</h5>
  ...
  <div class="row equal-height-row">
      ...
      <div class="card h-100 p-6">
        ...
        <div style="    min-height: 75px;" class="row">
          ...
          <div style="margin-top: -12px;" class="col-6">
            <label class="switch switch-lg">
              <input type="checkbox" @checked(old('habilitarAsistencias')) class="switch-input"
          ...
          <div style="    margin-top: -40px;" id="containesAsistenciasMinimas"
            class="mb-3 {{ old('habilitarAsistencias') ? '' : 'd-none' }} col-6">
            <label for="asistenciasMinimas" class="form-label">Asistencias Mínimas (opcional)</label>
            <input value="{{ old('asistenciasMinimas') }}" type="number" class="form-control"
        ...
        <div style="    min-height: 75px;" class="row">
          ...
          <div style="    margin-top: -40px;" id="containesAsistenciasAlerta"
            ...
            <input value="{{ old('cantidadInasistencias') }}" type="number" class="form-control"
        ...
        <div class="row">
          ...
          <div class="mb-3  col-12 col-md-6 ">
            ...
            <label class="switch switch-lg">
              <input type="checkbox" @checked(old('habilitarCalificaciones')) class="switch-input"
          ...
          <div class="mb-3   col-12 col-md-6">
            ...
            <label class="switch switch-lg">
              <input type="checkbox" @checked(old('habilitarTraslado')) class="switch-input"
          ...
          <div class="mb-3  col-12 col-md-6">
            ...
            <label class="switch switch-lg">
              <input type="checkbox" @checked(old('obligatorio')) class="switch-input"
      ...
      <div class="card h-100 p-6">
        ...
        <div class="col-12 col-md-12 mb-3"> {{-- Añadido mb-3 --}}
          ...
          <select class="form-select select2" name="paso_iniciar_id"
            ...
            <option value="{{ $paso->id_paso }}|{{ $paso->estado }}"
              {{ old('paso_iniciar_id') == $paso->id_paso . '|' . $paso->estado ? 'selected' : '' }}>
              {{ $paso->nombre }}
            </option>
            @endforeach
          </select>
        ...
        <div class="col-12 col-md-12 mb-3"> {{-- Añadido mb-3 --}}
          ...
          <select class="form-select select2" name="paso_culminar_id"
            ...
            @foreach ($pasosCrecimiento as $paso)
            <option value="{{ $paso->id_paso }}|{{ $paso->estado }}"
              {{ old('paso_culminar_id') == $paso->id_paso . '|' . $paso->estado ? 'selected' : '' }}>
              {{ $paso->nombre }}
            </option>
            @endforeach
          </select>
        ...
        <div class="mb-3 col-12">
          ...
          <select id="prerrequisitos" class="form-select select2" name="prerrequisitos[]" multiple
            ...
            <option value="{{ $otroNivel->id }}"
              {{ is_array(old('prerrequisitos')) && in_array($otroNivel->id, old('prerrequisitos')) ? 'selected' : '' }}>
              {{ $otroNivel->nombre }}
            </option>
            @endforeach
          </select>
        ...
        <div class="mb-3 col-12">
          ...
          <select id="procesos_prerrequisito" class="form-select select2" name="procesos_prerrequisito[]"
            ...
            <option value="{{ $paso->id_paso }}|{{ $paso->estado }}" {{-- Revisa si necesitas la lógica compleja de preselección como en crear-materia, adaptada para 'old' --}}
              {{ is_array(old('procesos_prerrequisito')) && in_array($paso->id_paso . '|' . $paso->estado, old('procesos_prerrequisito')) ? 'selected' : '' }}>
              {{ $paso->nombre }}
            </option>
            @endforeach
          </select>
    ...
    <div class="col-12 mt-4">
      <div class="d-flex justify-content-start">
        <button type="submit" class="btn btn-primary rounded-pill me-2 btnGuardar">Guardar nivel</button>
