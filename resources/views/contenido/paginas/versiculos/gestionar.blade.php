        ...
        $(function() {
            ...
            if (modalVersiculo) {
                modalVersiculo.addEventListener('show.bs.modal', function(event) {
                    ...
                    document.getElementById('modalVersiculoTexto').innerHTML = contenido ||
                        'Sin contenido de versículo.';
                });
            }
...
@section('content')
    ...
    <form id="filter-form" action="{{ route('versiculos.index') }}" method="GET" class="row g-3 align-items-center mb-4">
        <div class="col-12 col-md-4">
            <label for="fecha_rango" class="form-label text-black fw-semibold text-uppercase"
                style="font-size: 0.75rem;">Filtrar por rango de fecha</label>
                                ...
                                @php
                                    ...
                                    if (isset($versiculo->texto_versiculo) && is_array($versiculo->texto_versiculo)) {
                                        foreach ($versiculo->texto_versiculo as $selection) {
                                            foreach ($selection['versiculos'] as $v) {
                                                $fullText .=
                                                    '<strong>' . $v['numero'] . '</strong> ' . $v['texto'] . '<br>';
                                            }
                                        }
                                    }
                                @endphp
                                <button type="button" class="btn p-0 btn-ver-versiculo text-black" title="Ver versículo"
                                    data-bs-toggle="modal" data-bs-target="#modalVersiculo"
                                    data-cita="{{ $versiculo->cita_larga }}" data-contenido="{{ $fullText }}">
                                    <i class="ti ti-book ti-sm cursor-pointer border-0"></i>
                                </button>
                                <div class="d-flex align-items-center gap-1 text-black">
                                    <i
                                        class="ti ti-heart ti-sm cursor-pointer @if ($versiculo->usuariosQueDieronLike()->count() > 0) text-danger @endif"></i>
                                    <span
                                        style="font-size: 0.8rem;">{{ $versiculo->usuariosQueDieronLike()->count() >= 1000 ? round($versiculo->usuariosQueDieronLike()->count() / 1000, 1) . 'K' : $versiculo->usuariosQueDieronLike()->count() }}</span>
                                </div>
                            </div>
                        </div>
        ...
        @empty
            <div class="col-12 w-100 mt-5">
                <div class="text-center py-5">
                    ...
                    <a href="{{ route('versiculos.create') }}" class="btn btn-primary mt-3 rounded-pill">Crear mi primer
                        versículo</a>
                </div>
            </div>
        @endforelse
            ...
            <div class="modal-content">
                ...
                <div class="modal-body px-5 py-2 position-relative">
                    <div id="ratioContainer"
                        ...
                        <iframe id="videoIframe" src="" title="Video reflexión" allowfullscreen
        ...
        <div class="modal-dialog modal-dialog-centered modal-lg" role="document">
            <div class="modal-content shadow-lg border-0">
                ...
                <div class="modal-body text-center pt-0 pb-5 px-4 overflow-auto" style="max-height: 80vh;">
                    ...
                    <div id="modalVersiculoTexto" class="fs-6 px-md-5 text-black text-start"
