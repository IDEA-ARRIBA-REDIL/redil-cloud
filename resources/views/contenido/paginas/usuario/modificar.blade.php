      ...
      <div class="card-body">
        <div class="row">
          ...
          <div class="mb-3 {{ $campo->pivot->class }}">
            ...
            <select id="entidad_relacionada" data-placeholder="{{ $campo->placeholder }}"
              name="{{ $campo->name_id }}" class="select2 form-select"
              data-allow-clear="true">
              <option value="">Ninguno</option>
              @foreach ($entidadesRelacionadas as $entidadRelacionada)
              <option value="{{ $entidadRelacionada->id }}"
                {{ old($campo->name_id, $usuario->entidad_relacionada_id) == $entidadRelacionada->id ? 'selected' : '' }}>
                {{ ucwords($entidadRelacionada->nombre) }}
              </option>
              @endforeach
            </select>
            <div id="error{{ $campo->name_id }}"></div>
