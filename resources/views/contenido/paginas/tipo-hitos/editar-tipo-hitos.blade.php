...
@section('title', 'Editar Tipo de Hito')
...
<div class="container-fluid mt-4">
    <div class="d-flex justify-content-between align-items-center mb-4">
        <div>
            <h4 class="mb-1 fw-bold text-dark d-flex align-items-center">
                <i class="ti ti-trophy me-2 text-primary"></i> Editar Tipo de Hito: <span class="ms-1 text-primary">{{ $tipoHito->nombre }}</span>
            </h4>
            <p class="text-muted mb-0">Modifica los parámetros de configuración y capacidades de este tipo de hito.</p>
        </div>
    ...
    <form id="formTipoHito" action="{{ route('tipo-hitos.actualizarTipoHitos', $tipoHito->id) }}" method="POST">
        @csrf
        @method('PUT')
                        ...
                        <div class="row g-3">
                            <div class="col-md-6">
                                ...
                                <input type="text" name="nombre" id="nombre" class="form-control @error('nombre') is-invalid @enderror"
                                       value="{{ old('nombre', $tipoHito->nombre) }}" placeholder="Ej: Reconocimiento Ministerial" required maxlength="100">
                                @error('nombre')
                            ...
                            <div class="col-md-6">
                                <label for="slug" class="form-label fw-semibold text-dark">Slug / Identificador <span class="text-danger">*</span></label>
                                <input type="text" name="slug" id="slug" class="form-control @error('slug') is-invalid @enderror"
                                       value="{{ old('slug', $tipoHito->slug) }}" required maxlength="50"
                                       {{ in_array($tipoHito->slug, ['general', 'automatico', 'actividad', 'manual']) ? 'readonly' : '' }}>
                                <small class="text-muted">
                                    @if(in_array($tipoHito->slug, ['general', 'automatico', 'actividad', 'manual']))
                                        <i class="ti ti-lock me-1 text-warning"></i> Slug protegido del sistema.
                                    @else
                            ...
                            <div class="col-md-6">
                                ...
                                <div class="input-group">
                                    <span class="input-group-text"><i id="previewIcon" class="{{ old('icono', $tipoHito->icono ?? 'ti ti-award') }}"></i></span>
                                    <input type="text" name="icono" id="icono" class="form-control @error('icono') is-invalid @enderror"
                                           value="{{ old('icono', $tipoHito->icono) }}" placeholder="ti ti-award" required>
                                </div>
                            ...
                            <div class="col-md-6">
                                ...
                                <div class="d-flex align-items-center gap-2">
                                    <input type="color" name="color" id="color" class="form-control form-control-color"
                                           value="{{ old('color', $tipoHito->color ?? '#7c5cfc') }}" style="width: 50px; height: 38px;">
                                    <input type="text" id="colorHex" class="form-control" value="{{ old('color', $tipoHito->color ?? '#7c5cfc') }}" maxlength="7">
                                </div>
                            ...
                            <div class="col-12">
                                ...
                                <textarea name="descripcion" id="descripcion" rows="3" class="form-control @error('descripcion') is-invalid @enderror"
                                          placeholder="Explica para qué sirve o cuándo se utiliza este tipo de hito...">{{ old('descripcion', $tipoHito->descripcion) }}</textarea>
                                @error('descripcion')
                                    <div class="invalid-feedback">{{ $message }}</div>
                                @enderror
                            </div>
                    ...
                    <div class="card-body p-4">
                        <div class="d-flex flex-column gap-3">
                            <div class="form-check form-switch">
                                <input class="form-check-input" type="checkbox" id="activo" name="activo" value="1" {{ old('activo', $tipoHito->activo) ? 'checked' : '' }}>
                            ...
                            <div class="form-check form-switch">
                                <input class="form-check-input" type="checkbox" id="requiere_trigger" name="requiere_trigger" value="1" {{ old('requiere_trigger', $tipoHito->requiere_trigger) ? 'checked' : '' }}>
                            ...
                            <div class="form-check form-switch">
                                <input class="form-check-input" type="checkbox" id="requiere_actividad" name="requiere_actividad" value="1" {{ old('requiere_actividad', $tipoHito->requiere_actividad) ? 'checked' : '' }}>
                            ...
                            <div class="form-check form-switch">
                                <input class="form-check-input" type="checkbox" id="permite_fotos_usuario" name="permite_fotos_usuario" value="1" {{ old('permite_fotos_usuario', $tipoHito->permite_fotos_usuario) ? 'checked' : '' }}>
                            ...
                            <div class="form-check form-switch">
                                <input class="form-check-input" type="checkbox" id="permite_likes" name="permite_likes" value="1" {{ old('permite_likes', $tipoHito->permite_likes) ? 'checked' : '' }}>
                            ...
                            <div class="form-check form-switch">
                                <input class="form-check-input" type="checkbox" id="evaluacion_dinamica" name="evaluacion_dinamica" value="1" {{ old('evaluacion_dinamica', $tipoHito->evaluacion_dinamica) ? 'checked' : '' }}>
