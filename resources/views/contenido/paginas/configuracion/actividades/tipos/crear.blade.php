...
<p class="mb-4 text-black">Configura los parámetros básicos para este tipo de actividad.</p>

@include('layouts.status-msn')

<form id="formulario" action="{{ route('gestionar-tipos-de-actividad.crear') }}" method="POST">
  @csrf
        ...
        <div class="card-body">
          <div class="row">
            <div class="col-md-8 col-12 mb-3">
              ...
              <input type="text" id="nombre" name="nombre" class="form-control" value="{{ old('nombre') }}" placeholder="Ej: Escuela de Líderes" required />
              @error('nombre')
            ...
            <div class="col-md-4 col-12 mb-3">
              ...
              <input type="color" id="color" name="color" class="form-control form-control-color w-100" value="{{ old('color', '#666CE8') }}" title="Elige un color" />
              @error('color')
                <div class="text-danger ti-12px mt-2"><i class="ti ti-circle-x"></i> {{ $message }}</div>
              @enderror
            </div>
            <div class="col-12">
              ...
              <textarea id="descripcion" name="descripcion" class="form-control" rows="3" placeholder="Escribe una breve descripción..." required>{{ old('descripcion') }}</textarea>
              @error('descripcion')
                <div class="text-danger ti-12px mt-2"><i class="ti ti-circle-x"></i> {{ $message }}</div>
              @enderror
            </div>
          ...
          <div class="row g-3">
            ...
            <div class="col-12 col-md-3">
              <div class="form-check form-switch">
                <input class="form-check-input" type="checkbox" id="{{ $config['id'] }}" name="{{ $config['id'] }}" {{ old($config['id']) ? 'checked' : '' }}>
      ...
      <div class="card mb-4">
        ...
        <div class="card-body">
          <div class="row g-3">
            ...
            <div class="col-12 col-md-3">
              <div class="form-check form-switch">
                <input class="form-check-input" type="checkbox" id="{{ $compra['id'] }}" name="{{ $compra['id'] }}" {{ old($compra['id']) ? 'checked' : '' }}>
      ...
      <div class="card mb-4">
        ...
        <div class="card-body">
          <div class="row g-3">
            ...
            <div class="col-12 col-md-3">
              <div class="form-check form-switch">
                <input class="form-check-input" type="checkbox" id="{{ $restriccion['id'] }}" name="{{ $restriccion['id'] }}" {{ old($restriccion['id']) ? 'checked' : '' }}>
