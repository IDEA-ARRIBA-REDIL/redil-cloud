...
class UserController extends Controller
{
        ...
        if ($usuario->save()) {
            ...
            foreach ($pasos_crecimiento as $paso) {
                CrecimientoUsuario::create([
                    ...
                    'estado_id' => 1,
                    'fecha' => now(),
                    'detalle' => 'Estado inicial por defecto al crear usuario',
                ]);
            }
        ...
        if ($rolActivo->hasPermissionTo('personas.panel_procesos_asistente') && $rolActivo->hasPermissionTo('personas.editar_procesos_asistente')) {
            ...
            foreach ($pasosCrecimiento as $paso) {
                ...
                CrecimientoUsuario::updateOrCreate(
                    ['user_id' => $usuario->id, 'paso_crecimiento_id' => $paso->id],
                    [
                        'estado_id' => $request['estado_paso_'.$paso->id] ?: 1,
                        'fecha' => ($request['estado_paso_'.$paso->id] != 1) ? $request['fecha_paso_'.$paso->id] : null,
    ...
    public function geoAsignacion(?int $formulario, User $usuario)
    {
        // Esta línea ejecuta la policy. Si devuelve false, se lanza un error 403 automáticamente.
    ...
    public function relacionesFamiliares(?int $formulario, User $usuario)
    {
