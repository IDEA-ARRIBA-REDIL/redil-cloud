...
class ActividadController extends Controller
{
    ...
    public function nueva()
    {
        $configuracion = Configuracion::find(1);
        $tiposActividad = TipoActividad::get();
        $tagsGenerales = TagGeneral::all();

        $actividades = Actividad::where('activa', true)->with('tags')->get();
        $arrayActividades = [];
        ...
        foreach ($actividades as $acti) {
            $arrayActividades[] =
              [
                  ...
                  'borderColor' => '#000000',
                  'extendedProps' => [
                      'tags' => $acti->tags->pluck('id')->toArray(),
                  ],
              ];
        }
    ...
    public function crear(Request $request)
    {
        ...
        $actividad->mostrar_en_proximas_actividades = $request->has('mostrar_en_proximas_actividades');
        $actividad->totalmente_publica = $request->has('totalmente_publica');
        $actividad->restriccion_por_categoria = $request->has('restriccion_por_categoria');

        if ($request->habilitadoPDP == 'on' || $request->habilitada_pdp == '1') {
    ...
    public function abonos(Actividad $actividad)
    {
        ...
        if ($tiposCargo->contains('pestana_abonos', true) || $rolActivo->hasPermissionTo('actividades.ver_todas_las_actividades')) {
            ...
            $categoriasActividad->map(function ($categoria) {
