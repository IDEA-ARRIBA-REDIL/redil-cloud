...
class GrupoController extends Controller
{
        ...
        if ($parametrosBusqueda->buscar != '') {
            ...
            foreach ($buscar_array as $palabra) {
                $grupos = $grupos->filter(function ($grupo) use ($palabra) {
                    return stristr(Helpers::sanearStringConEspacios($grupo->nombre), $palabra) !== false ||
                      stristr(Helpers::sanearStringConEspacios($grupo->direccion), $palabra) !== false ||
                      stristr(Helpers::sanearStringConEspacios($grupo->primer_nombre), $palabra) !== false ||
                      stristr(Helpers::sanearStringConEspacios($grupo->segundo_nombre), $palabra) !== false ||
                      stristr(Helpers::sanearStringConEspacios($grupo->primer_apellido), $palabra) !== false ||
                      stristr(Helpers::sanearStringConEspacios($grupo->segundo_apellido), $palabra) !== false ||
                      $grupo->id === $palabra;
                });
            }
    ...
    public function listadoFinalCsv(Request $request)
    {
        ...
        if ($configuracion->visible_seccion_campos_extra_grupo) {
            $arrayCamposExtra = $request->informacionCamposExtras ? $request->informacionCamposExtras : [];
        } // $arrayCamposExtra
    ...
    public function crear(Request $request)
    {
        ...
        if ($configuracion->visible_seccion_campos_extra_grupo == true) {
            ...
            foreach ($camposExtras as $campo) {
                if ($campo->tipo_de_campo != 4) {
                    $grupo->camposExtras()->attach($campo->id, ['valor' => ucwords(mb_strtolower($request[$campo->class_id]))]);
                } else {
                    $grupo->camposExtras()->attach($campo->id, ['valor' => (json_encode($request[$campo->class_id]))]);
                }
            }
        }
    ...
    private function getDashboardStats($fechaInicio, $fechaFin, $tiposSeleccionados, $bloquesSeleccionados, $sedesSeleccionadas)
    {
        ...
        $bloques = BloqueDashboardConsolidacion::with(['sedes' => function ($query) use ($tiposSeleccionados, $fechaFin) {
    ...
    public function perfilEstadisticasGrupo(Request $request, Grupo $grupo, User $encargado)
    {
        ...
        switch ($request->rango) {
            ...
            case 'otroSemanas':
                [$añoIni, $semanaIni] = explode('-W', $request->semanaIni);
                $fechaInicio = Carbon::now()->setISODate($añoIni, $semanaIni)->startOfWeek();

                [$añoFin, $semanaFin] = explode('-W', $request->semanaFin);
    ...
    public function graficoDelMinisterio($id_nodo = 'U-logueado', $maximos_niveles = 3)
    {
        ...
        $array_ids_usuarios_no_dibujados = [];
