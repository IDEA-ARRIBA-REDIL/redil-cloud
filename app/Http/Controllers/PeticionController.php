...
class PeticionController extends Controller
{
    ...
    {
        $rolActivo = auth()->user()->roles()->wherePivot('activo', true)->first();
        $rolActivo->verificacionDelPermiso('peticiones.opcion_eliminacion_masiva');
        $peticiones = [];
    ...
    public function eliminacion($id)
    {
        ...
        // verificar si cumple el permiso
        $rolActivo->verificacionDelPermiso('peticiones.opcion_eliminar');

        $peticion = Peticion::findOrFail($id);

        // Si no tiene permiso global, validar alcance de cobertura
        if (! $rolActivo->hasPermissionTo('peticiones.lista_peticiones_todas')) {
            if ($rolActivo->hasPermissionTo('peticiones.lista_peticiones_solo_ministerio')) {
                $idsPermitidos = auth()->user()->misPeticiones()->pluck('id')->all();
                if (! in_array($peticion->id, $idsPermitidos, true)) {
                    abort(403);
                }
            } else {
                abort(403);
