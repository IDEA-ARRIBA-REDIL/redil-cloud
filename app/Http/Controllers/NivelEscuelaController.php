...
use App\Models\NivelEscuela; // Asegúrate de importar Configuracion
...
class NivelEscuelaController extends Controller
{
    ...
    public function guardar(Request $request, Escuela $escuela)
    {
        ...
        $validatedData = $request->validate([
            ...
            'tipoUsuarioInicial' => 'nullable|integer|exists:tipo_usuario,id',
            'tipoUsuarioObjetivo' => 'nullable|integer|exists:tipo_usuario,id',
        ]);
        ...
        try {
            ...
            if ($request->filled('foto')) {
                ...
                if (str_contains($request->foto, ';base64,')) {
                    @[$type, $file_data] = explode(';', $request->foto);
    ...
    private function procesarPasoNivel(NivelEscuela $nivel, $pasoCompleto, $esInicio)
    {
        if (str_contains($pasoCompleto, '|')) {
            [$pasoId, $estado] = explode('|', $pasoCompleto);
