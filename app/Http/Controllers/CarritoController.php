...
use App\Models\Inscripcion; // Importar la clase Validator
...
class CarritoController extends Controller
{
    ...
    {
        // Aplicar middleware auth a todos los métodos excepto listado, perfil y flujos públicos de carrito
        ...
        foreach ($request->all() as $key => $value) {
            if (strpos($key, 'elemento-') === 0) {
                ...
                switch ($elemento->tipoElemento->clase) {
                    ...
                    case 'archivo':
                        if ($request->filled($key) && is_string($request->$key) && ! str_contains($request->$key, 'base64')) {
                            // Si ya se subió vía Alpine.js, recibimos el nombre del archivo
                            $respuesta->url_archivo = $request->$key;
                    ...
                    case 'imagen':
                        if ($request->filled($key) && is_string($request->$key) && ! str_contains($request->$key, 'base64')) {
                            // Si ya se subió vía Alpine.js, recibimos el nombre del archivo
                            $respuesta->url_foto = $request->$key;
                        } elseif ($request->filled($key) && str_contains($request->$key, 'base64')) {
                            // Validación manual de peso máximo (en MB)
                            if (isset($elemento->peso_maximo) && $elemento->peso_maximo > 0) {
    ...
    public function escuelasCarrito($compra_id, Actividad $actividad, $primeraVez)
    {
    ...
    /**
     * Recibe un archivo del formulario de actividades vía Alpine.js fetch (POST normal).
     * Este método evita usar Livewire WithFileUploads (que causa tempnam() en producción multi-tenant).
     * El archivo se guarda en Storage::disk('public') del tenant activo y retorna el nombre resultante.
     */
    public function uploadArchivoFormulario(Request $request): \Illuminate\Http\JsonResponse
    {
