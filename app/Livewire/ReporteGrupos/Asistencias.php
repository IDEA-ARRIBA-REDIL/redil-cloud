...
class Asistencias extends Component
{
    ...
    private function inicarAsistentesAlGrupo()
    {
        ...
        $asistentesReporte = $this->reporte->usuarios()
            ->withTrashed()
            ->select('users.id', 'foto', 'genero', 'email', 'primer_nombre', 'segundo_nombre', 'primer_apellido', 'segundo_nombre', 'identificacion', 'tipo_identificacion_id', 'tipo_usuario_id', 'asistencia_grupos.asistio', 'asistencia_grupos.tipo_inasistencia_id', 'asistencia_grupos.observaciones')
            ->get();
        ...
        $asistentesActuales = $this->grupo->asistentes()
            ...
            ->select('users.id', 'foto', 'genero', 'email', 'primer_nombre', 'segundo_nombre', 'primer_apellido', 'segundo_nombre', 'identificacion', 'tipo_identificacion_id', 'tipo_usuario_id')
            ->get();
    ...
    private function iniciarOfrendasGenericas()
    {
        ...
        foreach ($tiposOfrendasGenericas as $tipoOfrendaGenerica) {
            ...
            $valorActual = $ofrendaTemporal ? number_format((float) $ofrendaTemporal->valor, 2, '.', '') : '';
            ...
            $this->inputsOfrendasGenerica[$tipoOfrendaGenerica->id]['valor'] = $valorActual;
        ...
        if (strlen($this->search) > 2) {
            ...
            foreach ($buscar_array as $palabra) {
                $this->personasFiltradas = $this->personasFiltradas->filter(function ($persona) use ($palabra) {
                    $respuesta = stristr(Helpers::sanearStringConEspacios($persona->primer_nombre), $palabra) !== false ||
                      stristr(Helpers::sanearStringConEspacios($persona->segundo_nombre), $palabra) !== false ||
                      stristr(Helpers::sanearStringConEspacios($persona->primer_apellido), $palabra) !== false ||
                      stristr(Helpers::sanearStringConEspacios($persona->segundo_apellido), $palabra) !== false ||
                      stristr(Helpers::sanearStringConEspacios($persona->identificacion), $palabra) !== false ||
                      stristr(Helpers::sanearStringConEspacios($persona->email), $palabra) !== false;
    ...
    public function abrirModalOfrendaEspecifica($userId)
    {
        ...
        foreach ($tiposOfrenda as $tipoOfrenda) {
            $valorActual = isset($ofrendasActuales[$tipoOfrenda->id]['valor']) ? number_format((float) $ofrendasActuales[$tipoOfrenda->id]['valor'], 2, '.', '') : '0.00';
    ...
    {
        // Reemplazar comas por puntos en los valores ingresados para admitir formatos de entrada europeos e hispanos
        foreach ($this->inputsTemporalOfrendasEspecificasUsuario as $tipoId => $datos) {
            if (isset($datos['valor'])) {
                $this->inputsTemporalOfrendasEspecificasUsuario[$tipoId]['valor'] = str_replace(',', '.', $datos['valor']);
            }
        }
        ...
        if ($this->dador) {
            ...
            $this->ofrendasPorPersona[$userId] = collect($this->inputsTemporalOfrendasEspecificasUsuario)
                ...
                ->mapWithKeys(function ($item, $key) {
                    ...
                    $valorRedondeado = round((float) $item['valor'], 2);
                    ...
                    return [
                        $key => [
                            ...
                            'tipo_ofrenda_id' => $key,
                            'valor' => $valorRedondeado > 0 ? $valorRedondeado : 0,
                        ],
                    ];
                })
                ->toArray();
    ...
    public function submitFormulario()
    {
        ...
        if ($this->reporte->aprobado === null) {
            foreach ($this->inputsOfrendasGenerica as $tipoOfrendaId => $datosOfrenda) {
                $valorGenerica = round((float) str_replace(',', '.', $datosOfrenda['valor'] ?? 0), 2);
                $ofrendaGenerica = $this->reporte->ofrendas()->where('tipo_ofrenda_id', '=', $tipoOfrendaId)->first();

                if ($ofrendaGenerica) {
                    if ($valorGenerica > 0) {
                        $ofrendaGenerica->valor = $valorGenerica;
                        ...
                        if (! $this->configuracion->tiene_sistema_aprobacion_de_reporte) {
                            $ofrendaGenerica->valor_real = $valorGenerica;
                        }
                        ...
                        if ($ingreso) {
                            $ingreso->valor = $valorGenerica;
                            if (! $this->configuracion->tiene_sistema_aprobacion_de_reporte) {
                                $ingreso->valor_real = $valorGenerica;
                            }
                            $ingreso->save();
                        }
                    ...
                    if ($valorGenerica > 0) {
                        ...
                        $ofrendaGenericaNueva->valor = $valorGenerica;
                        ...
                        if (! $this->configuracion->tiene_sistema_aprobacion_de_reporte) {
                            $ofrendaGenericaNueva->valor_real = $valorGenerica;
                            $ofrendaGenericaNueva->save();
                            $ingreso->valor_real = $valorGenerica;
                        }
        ...
        if ($this->reporte->aprobado === null) {
            ...
            foreach ($this->ofrendasPorPersona as $userId => $ofrendas) {
                ...
                foreach ($ofrendas as $tipoOfrendaId => $ofrendaData) {
                    $valorEspecifica = round((float) str_replace(',', '.', $ofrendaData['valor'] ?? 0), 2);
                    ...
                    if ($ofrendaActual) {
                        if ($valorEspecifica > 0) {
                            $ofrendaActual->valor = $valorEspecifica;
                            // actualiza las ofrendas automaticamente si no hay sistema de aprobacion
                            if (! $this->configuracion->tiene_sistema_aprobacion_de_reporte) {
                                $ofrendaActual->valor_real = $valorEspecifica;
                            }
                            $ofrendaActual->save();
                            ...
                            if ($ingreso) {
                                $ingreso->valor = $valorEspecifica;
                                if (! $this->configuracion->tiene_sistema_aprobacion_de_reporte) {
                                    $ingreso->valor_real = $valorEspecifica;
                                }
                                $ingreso->save();
                            }
                        ...
                        if ($valorEspecifica > 0) {
                            ...
                            $ofrendaNueva->valor = $valorEspecifica;
                            ...
                            // aprueba las ofrendas automaticamente si no hay sistema de aprobacion
                            if (! $this->configuracion->tiene_sistema_aprobacion_de_reporte) {
                                $ofrendaNueva->valor_real = $valorEspecifica;
                                $ofrendaNueva->save();
                                $ingreso->valor_real = $valorEspecifica;
                            }
        ...
        try {
            ...
            NotificacionService::dispatch('grupo_reporte_finalizado', [
                'titulo' => 'Reporte Finalizado',
                'mensaje' => "Se ha reportado asistencia y ofrendas para el grupo: {$grupoName}",
                'icono' => 'ti-file-check',
                'url' => route('reporteGrupo.resumen', $this->reporte->id),
                'color' => 'success',
            ], auth()->user());
        } catch (\Exception $e) {
            \Illuminate\Support\Facades\Log::error('Error al despachar notificación grupo_reporte_finalizado: '.$e->getMessage());
        }
