...
// 1. Importa el trait de paginación
...
class GestionarZonas extends Component
{
    ...
    public function eliminarZona($id)
    {
        ...
        $this->dispatch(
            'msn',
            msnIcono: 'success',
            msnTitulo: '¡Zona dada de baja!',
            msnTexto: 'La zona ha sido enviada a la papelera.',
        );
    }
    ...
    public function restaurarZona($id)
    {
        ...
        $this->dispatch(
            'msn',
            msnIcono: 'success',
            msnTitulo: '¡Zona Restaurada!',
            msnTexto: 'La zona ha sido restaurada con éxito.',
        );
    }

    #[On('eliminarPermanenteZona')]
    public function eliminarPermanenteZona($id)
    {
        $zona = Zona::withTrashed()->findOrFail($id);
        $zona->forceDelete();
        ...
        $this->dispatch(
            ...
            msnTexto: 'La zona ha sido eliminada permanentemente del sistema.',
        );
    }
    ...
    {
        $query = Zona::with('sedes', 'localidades');

        if ($this->conEliminados) {
            $query->onlyTrashed();
        }

        $zonas = $query->when($this->busqueda, function ($query) {
