...
class Biblia extends Component
{
    ...
    public function mount()
    {
        ...
        try {
            $responseVersion = Http::withOptions([
                'timeout' => 5, // Establece un tiempo de espera de 5 segundos
            ])->get('https://api.midvash.com/v1/versions');
            ...
            if ($responseVersion->failed()) {
                ...
                $rawVersions = collect($responseVersion->json()['data'] ?? []);
                $mappedVersions = $rawVersions->map(function ($v) {
                    return [
                        'name' => $v['name'],
                        'version' => $v['slug'],
                        'shortName' => $v['shortName'] ?? strtoupper($v['slug']),
                        'language' => $v['language'] ?? 'es',
                    ];
                });
                $spanishVersions = $mappedVersions->where('language', 'es');
                $this->versiones = $spanishVersions->isNotEmpty() ? $spanishVersions->values() : $mappedVersions->values();
            }
        } catch (\Exception $e) {
            $this->dispatch(
                'msn',
    ...
    {
        if ($value && $this->libroSeleccionado && $this->versionSeleccionada) {
            $this->subrayadosTemp = [];
            $bookSlug = strtolower($this->libroSeleccionado->names[0]);
            $versionSlug = $this->versionSeleccionada['version'];

            $url = 'https://api.midvash.com/v1/'.$versionSlug.'/'.$bookSlug.'/'.$value;
            ...
            try {
                ...
                if ($response->failed()) {
                    ...
                    if (! empty($data['verses']) && is_array($data['verses'])) {
                        foreach ($data['verses'] as $index => $verseText) {
                            $versesFormatted[] = [
                                'number' => $index + 1,
                                'verse' => $verseText,
                            ];
    ...
    public function updatedSelectVersion($value)
    {
        ...
        $this->updatedSelectCapitulo($this->capituloSeleccionado['chapter'] ?? 1);
    }
