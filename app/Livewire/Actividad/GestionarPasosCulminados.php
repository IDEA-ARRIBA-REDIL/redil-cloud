...
class GestionarPasosCulminados extends Component
{
    ...
    private function cargarDatos()
    {
        ...
        $this->pasosCulminados = $this->actividad->procesosCulminados()
            ->orderBy('actividad_procesos_culminados.indice')
            ->get();
    }
    ...
    public function agregarPaso()
    {
        ...
        $existente = $this->actividad->procesosCulminados()
        ...
        $maxIndice = $this->actividad->procesosCulminados()->max('indice') ?? 0;

        // Agregar el paso con el nuevo campo FK
        $this->actividad->procesosCulminados()->attach($this->pasoSeleccionado, [
    ...
    {
        $this->actividad->procesosCulminados()->detach($pasoId);
        
        // Reordenar índices
        $pasos = $this->actividad->procesosCulminados()->orderBy('indice')->get();
        foreach ($pasos as $index => $paso) {
            $this->actividad->procesosCulminados()->updateExistingPivot($paso->id, [
                'indice' => $index + 1
            ]);
        }
    ...
    public function actualizarOrden($ordenes)
    {
        foreach ($ordenes as $orden) {
            $this->actividad->procesosCulminados()->updateExistingPivot($orden['id'], [
                'indice' => $orden['orden']
            ]);
        }
    ...
    {
        return view('livewire.actividad.gestionar-pasos-culminados');
    }
}
