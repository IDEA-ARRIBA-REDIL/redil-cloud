...
class ResumenFinanciero extends Component
{
    ...
    public function abrirModalEdicion($tipoOfrendaId)
    {
        ...
        $this->valorModal = $this->ofrendaModalGenerica ? number_format((float) $this->ofrendaModalGenerica->valor, 2, '.', '') : '0.00';
    ...
    public function abrirModalOfrendaEspecifica($personaId)
    {
        ...
        foreach ($tiposOfrenda as $tipoOfrenda) {
            ...
            $valorActual = $ofrendaTemp ? number_format((float) $ofrendaTemp->valor, 2, '.', '') : '0.00';
    ...
    {
        // Reemplazar comas por puntos en valorModal para admitir formatos de entrada europeos e hispanos
        $this->valorModal = str_replace(',', '.', $this->valorModal);

        $this->validate([
            'valorModal' => 'required|numeric|min:0',
        ]);

        $valorRedondeado = round((float) $this->valorModal, 2);
        ...
        if ($this->ofrendaModalGenerica) {
            ...
            if ($valorRedondeado == 0) {
                ...
                $this->dispatch(
                    ...
                    msnTitulo: '¡Muy bien!',
                    timer: 1500,
                    msnTexto: 'La ofrenda fue eliminada correctamente'
                );

                $this->dispatch('cerrarModal', nombreModal: 'modalEdicionOfrenda');
            } elseif ($valorRedondeado > 0) {
                $this->ofrendaModalGenerica->valor = $valorRedondeado;
                $this->ofrendaModalGenerica->valor_real = $valorRedondeado;
                $this->ofrendaModalGenerica->save();

                $ingreso = Ingreso::where('ofrenda_id', $this->ofrendaModalGenerica->id)->first();
                if ($ingreso) {
                    $ingreso->valor_real = $valorRedondeado;
                    $ingreso->valor = $valorRedondeado;
                    $ingreso->save();
                }
                ...
                $this->dispatch(
                    ...
                    msnTitulo: '¡Muy bien!',
                    timer: 1500,
                    msnTexto: 'La ofrenda fue editada con éxito'
                );
            ...
            if ($valorRedondeado > 0) {
                ...
                $ofrenda->valor = $valorRedondeado;
                $ofrenda->valor_real = $valorRedondeado;
                ...
                $ingreso->identificacion = 'Reporte reunión: '.$this->reporteReunion->reunion->nombre;
                ...
                $this->dispatch(
                    ...
                    msnTitulo: '¡Muy bien!',
                    timer: 1500,
                    msnTexto: 'La ofrenda fue creada con éxito'
                );
    ...
    {
        // Reemplazar comas por puntos antes de validar y guardar
        ...
        foreach ($this->inputsTemporalOfrendasEspecificasUsuario as $tipoId => $datos) {
            ...
            if ($ofrendaTemporal) {
                if ($valorRedondeado == 0) {
                    ...
                    // Eliminar el ingreso asociado en la base de datos
                    $ingreso = Ingreso::where('ofrenda_id', $ofrendaTemporal->id)->first();
                    if ($ingreso) {
                        $ingreso->delete();
                    }

                    $ofrendaTemporal->delete();
                } elseif ($valorRedondeado > 0 && ($valorRedondeado != $ofrendaTemporal->valor)) {
                    $ofrendaTemporal->valor = $valorRedondeado;
                    $ofrendaTemporal->valor_real = $valorRedondeado;
                    $ofrendaTemporal->save();

                    // Sincronizar el ingreso asociado en la base de datos
                    $ingreso = Ingreso::where('ofrenda_id', $ofrendaTemporal->id)->first();
                    if ($ingreso) {
                        $ingreso->valor = $valorRedondeado;
                        $ingreso->valor_real = $valorRedondeado;
                    ...
                    $this->dispatch(
                        ...
                        msnTitulo: '¡Muy bien!',
                        timer: 1500,
                        msnTexto: 'Las ofrendas de '.$this->usuarioSeleccionado->nombre(3).' fueron editadas con éxito'
                    );
                }
            } else {
                if ($valorRedondeado > 0) {
                    ...
                    $ingreso->identificacion = $this->usuarioSeleccionado->identificacion;
                    ...
                    $this->dispatch(
                        ...
                        msnTitulo: '¡Muy bien!',
                        timer: 1500,
                        msnTexto: 'Las ofrendas de '.$this->usuarioSeleccionado->nombre(3).' fueron creadas con éxito'
                    );
                }
            }
        }
