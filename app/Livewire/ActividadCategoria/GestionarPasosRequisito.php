...
class GestionarPasosRequisito extends Component
{
    ...
    private function cargarDatos()
    {
        ...
        $this->pasosRequisito = $this->categoria->procesosRequisito()
            ->orderBy('actividad_categoria_procesos_requisitos.indice')
            ->get();
    }
    ...
    public function agregarPaso()
    {
        ...
        $existente = $this->categoria->procesosRequisito()
        ...
        $maxIndice = $this->categoria->procesosRequisito()->max('indice') ?? 0;

        // Agregar el paso con el nuevo campo FK
        $this->categoria->procesosRequisito()->attach($this->pasoSeleccionado, [
    ...
    {
        $this->categoria->procesosRequisito()->detach($pasoId);
        
        // Reordenar índices
        $pasos = $this->categoria->procesosRequisito()->orderBy('indice')->get();
        foreach ($pasos as $index => $paso) {
            $this->categoria->procesosRequisito()->updateExistingPivot($paso->id, [
                'indice' => $index + 1
            ]);
        }
    ...
    public function actualizarOrden($ordenes)
    {
        foreach ($ordenes as $orden) {
            $this->categoria->procesosRequisito()->updateExistingPivot($orden['id'], [
                'indice' => $orden['orden']
            ]);
        }
    ...
    {
        return view('livewire.actividad-categoria.gestionar-pasos-requisito');
    }
}
